...
@interface MetalRawdataRenderer () <ZoomVideoSDKRawDataPipeDelegate>
...
@interface MetalViewController () <ZoomVideoSDKDelegate, BottomBarViewDelegate, ChatInputViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIScrollViewDelegate>
...
- (void)noVideofailBack {
    ...
    if (fullRenderer) {
        ...
        if (user.getSharePipe.shareStatus.sharingStatus == ZoomVideoSDKReceiveSharingStatus_Start) {
...
- (MetalRawdataRenderer *)getThumbRendererForUser:(ZoomVideoSDKUser *)user
{
    for (MetalRawdataRenderer *renderer in self.rendererArr) {
...
- (void)onUserShareStatusChanged:(ZoomVideoSDKShareHelper *)helper user:(ZoomVideoSDKUser *)user status:(ZoomVideoSDKReceiveSharingStatus)status
{
    ...
    if (status == ZoomVideoSDKReceiveSharingStatus_Stop ||
        ...
        if (!user.getVideoPipe.videoStatus.on && (user.getSharePipe.shareStatus.sharingStatus != ZoomVideoSDKReceiveSharingStatus_Start)) {
